...
path.script = "~/Dropbox/_SPACE/DREAM9.5/"
setwd(path.script)
library(neuralnet)
require(FSelector) 
require(e1071)
...
for(cv in CV) #Begin Cross-Fold for Validation or for Model Tuning
{
  ...
  for (curr.study in STUDY)  ## LOOP THROUGH THE DATA FRAMES
  {
    ...
    ##### FEATURE SELECTION ##############################################################
    ##### RF
    # use.all.features <- colnames(curr.training.data[,setdiff(names(table.for.validation),c("RPT","STUDYID"))])
    ...
    # hill.climbing         <- hill.climbing.search(features, evaluatorrf)
    #    

    
    #### SAVED FEATURES FROM PREVIOUS RUN
    ...
    prob              <- predict(model.rf, curr.testing.data, type = "prob")[,2] # nb: 'class', rf: 'prob'
    val.prob          <- predict(model.rf, table.for.validation,type="prob")[,2] # nb: 'class', rf: 'prob'
    
    # NB
    # model.nb          <- naiveBayes(x=x, y=y, type='prob')
    # prob              <- predict(model.nb, curr.testing.data, type = "raw")[,2] # nb: 'class', rf: 'prob'
    # val.prob          <- predict(model.nb, table.for.validation,type="raw")[,2] # nb: 'class', rf: 'prob'

    # library(class) 
    # model.knn         <- knn(curr.training.data[, features], curr.testing.data[, features], cl=factor(y), k = length(features), l = 0, prob = TRUE, use.all = TRUE)
    # prob              <- attr(model.knn, 'prob')
    # model.knn         <- knn(curr.training.data[, features], table.for.validation[, features], cl=factor(y), k = length(features), l = 0, prob = TRUE, use.all = TRUE)
    # val.prob          <- attr(model.knn, 'prob')
    # 
    
    
    # model.nn            <- nnet(as.formula(paste("y ~ ", paste(features, collapse= "+"))), data=cbind(x,y), linear.output=FALSE, likelihood=TRUE, size=length(features))
    # prob                <- predict(model.nn, curr.testing.data)
    # val.prob            <- predict(model.nn, table.for.validation)
    # 
    
    model.svm           <- svm(as.formula(paste("y ~ ", paste(features, collapse= "+"))), data=cbind(x,y), probability=TRUE)
    prob                <- attr(predict(model.svm, curr.testing.data, probability=TRUE), 'probabilities')[,'X1']
    val.prob            <- attr(predict(model.svm, table.for.validation, probability=TRUE), 'probabilities')[, 'X1']
...
write.csv(SCORING.TABLE, file = paste("Data/NB/SCORE-svm-RF4-",testing.name,"-",ACC,".csv", sep=""))
